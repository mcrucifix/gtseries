...
#'
#' @param x The data provided either as a time series (advised), or as a vector. 
#' may be complex
...
#' @export mmfft
mmfft <- function(x, seglength = length(x) %/% 16, ...){
  N <- length(x)
  nsections <- (N %/% seglength) 
  N0 <- nsections * seglength
#   remainder  <- seglength - N0
  x0 <- matrix(x[1:N0], byrow=FALSE, seglength, nsections)
  myMfft <- function(x) mfft(x, ...)
  OUT <- apply(x0, 2, myMfft)
  class(OUT) <- "mmfft"
  attr(OUT, "nsections") <- nsections
  attr(OUT, "seglength") <- seglength
  attr(OUT, "start") <- ifelse(is.ts(x), stats::start(x), 1)
  attr(OUT, "deltat") <- ifelse(is.ts(x), stats::deltat(x), 1)
  return(OUT)
}
